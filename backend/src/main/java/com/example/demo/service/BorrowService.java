...
@CacheConfig(cacheNames = { "borrow_pending", "borrow_history" })
public class BorrowService {
        ...
        @CacheEvict(value = "borrow_pending", allEntries = true)
        @Transactional
        public BorrowResponse createBorrowRequest(BorrowRequest request) {
                log.info("[Cache EVICT] createBorrowRequest - clearing borrow pending cache");
                ...
                if (!user.isActive()) {
                        System.err.println("[BACKEND] Không thể tạo yêu cầu mượn vì tài khoản bị khóa - username="
                                        + username);
        ...
        public BorrowResponse createBorrowByAdmin(AdminBorrowRequest request) {
                System.out.println("[BACKEND] Bắt đầu tạo phiếu mượn trực tiếp bởi admin - userId="
                                + request.getUserId()
                ...
                if (!user.isActive()) {
                        System.err.println(
                                        "[BACKEND] Không thể tạo phiếu mượn trực tiếp vì user bị khóa - userId="
                                                        + request.getUserId());
                ...
                for (BorrowRequest.BorrowItemLine itemLine : request.getItems()) {
                        ...
                        if (inventory.getAvailableQuantity() < itemLine.getQuantity()) {
                                System.err.println("[BACKEND] Tồn kho không đủ khi tạo phiếu trực tiếp - bookId="
                                                + itemLine.getBookId() + ", con=" + inventory.getAvailableQuantity()
                                                + ", yeuCau="
        ...
        @CacheEvict(value = { "borrow_pending", "borrow_history", "inventory" }, allEntries = true)
        @Transactional
        public BorrowResponse approveBorrow(Long id, String adminNote) {
                log.info("[Cache EVICT] approveBorrow(id={}) - clearing borrow pending + history + inventory", id);
                ...
                if (!STATUS_PENDING.equals(borrowRecord.getStatus())) {
                        System.err.println("[BACKEND] Không thể duyệt phiếu do sai trạng thái - borrowId=" + id
                                        + ", trangThai="
                ...
                if (!borrowRecord.getUser().isActive()) {
                        System.err.println(
                                        "[BACKEND] Không thể duyệt phiếu vì user bị khóa - borrowId=" + id + ", userId="
                ...
                for (BorrowItem item : items) {
                        Inventory inventory = inventoryRepository.findByBook_Id(item.getBook().getId())
                                        .orElseThrow(() -> new RuntimeException(
                                                        "Không tìm thấy tồn kho cho sách id: "
                                                                        + item.getBook().getId()));

                        if (inventory.getAvailableQuantity() < item.getQuantity()) {
                                System.err.println("[BACKEND] Tồn kho không đủ khi duyệt phiếu - borrowId=" + id
                                                + ", bookId="
                                                + item.getBook().getId() + ", con=" + inventory.getAvailableQuantity()
                                                + ", yeuCau="
                                                + item.getQuantity());
                                throw new RuntimeException(
                                                "Sách \"" + item.getBook().getTitle()
                                                                + "\" không đủ tồn kho để duyệt (còn "
                                                                + inventory.getAvailableQuantity() + " cuốn, yêu cầu "
                                                                + item.getQuantity() + ")");
                        }
        ...
        @CacheEvict(value = { "borrow_pending", "borrow_history" }, allEntries = true)
        @Transactional
        public BorrowResponse rejectBorrow(Long id, String adminNote) {
                log.info("[Cache EVICT] rejectBorrow(id={}) - clearing borrow pending + history", id);
        ...
        @CacheEvict(value = { "borrow_history", "inventory" }, allEntries = true)
        @Transactional
        public BorrowResponse returnBorrow(Long id, ReturnRequest returnRequest) {
                log.info("[Cache EVICT] returnBorrow(id={}) - clearing borrow history + inventory", id);
                ...
                if (!STATUS_BORROWING.equals(borrowRecord.getStatus())) {
                        System.err.println(
                                        "[BACKEND] Không thể trả sách do trạng thái phiếu không hợp lệ - borrowId=" + id
                                                        + ", trangThai=" + borrowRecord.getStatus());
                ...
                for (ReturnRequest.ReturnItemLine x : returnRequest.getItems()) {
                        BorrowItem borrowItem = borrowItemRepository
                                        ...
                                        .orElseThrow(() -> new RuntimeException(
                                                        "Sách id " + x.getBookId() + " không thuộc phiếu mượn id "
                                                                        + id));

                        int canReturn = borrowItem.getQuantity() - borrowItem.getReturnedQuantity();
                        if (x.getReturnQuantity() > canReturn) {
                                throw new RuntimeException(
                                                "Sách id " + x.getBookId() + " chỉ còn có thể trả tối đa " + canReturn
                                                                + " cuốn");
                        }
                }
        ...
        @Cacheable(value = "borrow_pending", key = "'borrow_pending_all'")
        public List<BorrowResponse> getPendingBorrowRequests() {
                log.info("[Cache MISS] getPendingBorrowRequests - fetching from DB");
                List<BorrowResponse> responses = borrowRecordRepository.findByStatusOrderByCreatedAtAsc(STATUS_PENDING)
                                .stream()
                                .map(record -> {
                                        List<BorrowItemResponse> itemResponses = borrowItemRepository
                                                        .findByBorrowRecord_Id(record.getId())
                                                        .stream()
                                                        .map(BorrowItemResponse::fromEntity)
                                                        .toList();
                                        return BorrowResponse.builder()
        ...
        @Cacheable(value = "borrow_history", key = "'borrow_my_' + #username")
        public List<BorrowResponse> getMyBorrowRequests(String username) {
                log.info("[Cache MISS] getMyBorrowRequests(username={}) - fetching from DB", username);
                User user = userRepository.findByUsername(username)
                                .orElseThrow(() -> new RuntimeException("Không tìm thấy người dùng: " + username));
                ...
                List<BorrowResponse> responses = borrowRecordRepository.findByUser_IdOrderByCreatedAtDesc(user.getId())
                                ...
                                .toList();
                System.out.println(
                                "[BACKEND] Lấy danh sách phiếu mượn của người dùng - username=" + username + ", tong="
                                                + responses.size());
                return responses;
        }
        ...
        @Cacheable(value = "borrow_history", key = "'borrow_history_' + #username")
        public Page<BorrowResponse> getBorrowHistory(String username, Pageable pageable) {
                log.info("[Cache MISS] getBorrowHistory(username={}) - fetching from DB", username);
                User user = userRepository.findByUsername(username)
                                .orElseThrow(() -> new RuntimeException("Không tìm thấy người dùng: " + username));
        ...
        @Cacheable(value = "borrow_history", key = "'borrow_user_' + #username + '_borrowing'")
        public Page<BorrowResponse> getCurrentBorrows(String username, Pageable pageable) {
                log.info("[Cache MISS] getCurrentBorrows(username={}) - fetching from DB", username);
                User user = userRepository.findByUsername(username)
                                .orElseThrow(() -> new RuntimeException("Không tìm thấy người dùng: " + username));
                return borrowRecordRepository.findByUser_IdAndStatus(user.getId(), STATUS_BORROWING, pageable)
                                .map(record -> {
                                        List<BorrowItem> items = borrowItemRepository
                                                        .findByBorrowRecord_Id(record.getId());
                                        List<BorrowItemResponse> itemResponses = new ArrayList<>();
                                        for (BorrowItem it : items) {
                                                itemResponses.add(BorrowItemResponse.fromEntity(it));
        ...
        @Cacheable(value = "borrow_history", key = "'borrow_' + #id")
        public BorrowResponse getBorrowById(Long id) {
                log.info("[Cache MISS] getBorrowById(id={}) - fetching from DB", id);
                BorrowRecord borrowRecord = borrowRecordRepository.findById(id)
