...
> **Công nghệ:** Java 17 · Spring Boot · Spring Security · JWT · Spring Data JPA · MySQL  
...
Bảo mật được thực hiện thông qua **Spring Security + JWT** (access token/refresh token) và mã hóa mật khẩu bằng **BCrypt**. Dữ liệu trả về được chuẩn hóa qua lớp DTO (Data Transfer Object) nhằm tách biệt hoàn toàn model nội bộ khỏi API công khai.
...
| 4   | **Người dùng**            | User                 | Cá nhân đã đăng ký tài khoản trong hệ thống. Người dùng được gán vai trò phù hợp để truy cập chức năng.                                                             |
| 5   | **Vai trò**               | Role                 | Nhóm phân quyền xác định khả năng thao tác của người dùng. Hệ thống hiện có ba vai trò chính: `ADMIN`, `LIBRARIAN`, `USER`.                                         |
...
## 5. Cập nhật tài liệu theo phiên bản hiện tại

### 5.1. Cập nhật phạm vi nghiệp vụ

So với bản thu thập yêu cầu ban đầu, hệ thống hiện tại đã mở rộng các luồng sau:

- Xác thực theo JWT với các API đăng nhập, làm mới token và đăng xuất.
- Bổ sung vai trò `LIBRARIAN` bên cạnh `ADMIN` và `USER`.
- Bổ sung luồng mượn online theo yêu cầu: gửi yêu cầu, xem đơn của tôi, duyệt/từ chối yêu cầu.
- Bổ sung nhóm báo cáo thống kê: top sách, top người mượn, sách đang mượn, phiếu quá hạn.

### 5.2. Danh mục Use Case cập nhật

| ID   | Tên Use Case             | Actor chính            |
| ---- | ------------------------ | ---------------------- |
| UC01 | Đăng ký tài khoản        | Khách                  |
| UC02 | Đăng nhập                | Khách, Người dùng      |
| UC03 | Làm mới token            | Người dùng đã xác thực |
| UC04 | Đăng xuất                | Người dùng đã xác thực |
| UC05 | Xem danh mục sách        | Khách, Người dùng      |
| UC06 | Xem chi tiết sách        | Khách, Người dùng      |
| UC07 | Tìm kiếm sách            | Khách, Người dùng      |
| UC08 | Xem hồ sơ cá nhân        | Người dùng đã xác thực |
| UC09 | Cập nhật hồ sơ cá nhân   | Người dùng đã xác thực |
| UC10 | Đổi mật khẩu             | Người dùng đã xác thực |
| UC11 | Quản lý sách             | Admin, Librarian       |
| UC12 | Quản lý tác giả          | Admin, Librarian       |
| UC13 | Quản lý thể loại         | Admin, Librarian       |
| UC14 | Quản lý tồn kho          | Admin, Librarian       |
| UC15 | Xem nhật ký tồn kho      | Admin, Librarian       |
| UC16 | Tạo phiếu mượn trực tiếp | Admin, Librarian       |
| UC17 | Gửi yêu cầu mượn sách    | User                   |
| UC18 | Xem đơn mượn của tôi     | User                   |
| UC19 | Duyệt yêu cầu mượn       | Admin                  |
| UC20 | Từ chối yêu cầu mượn     | Admin                  |
| UC21 | Trả sách                 | Admin, Librarian       |
| UC22 | Xem danh sách phiếu mượn | Admin, Librarian       |
| UC23 | Xem chi tiết phiếu mượn  | Admin, Librarian       |
| UC24 | Quản lý người dùng       | Admin                  |
| UC25 | Xem báo cáo thống kê     | Admin                  |

### 5.3. Tài liệu UC Spec

Đặc tả chi tiết cho toàn bộ UC được tách thành tài liệu riêng tại:

- `backend/UC_SPEC.md`

Tài liệu này là nguồn tham chiếu chính cho: tiền điều kiện, hậu điều kiện, luồng chính, luồng ngoại lệ và quy tắc nghiệp vụ của từng UC.

_Hết tài liệu thu thập yêu cầu người dùng._
